!-------------------------------------------------------------------------
! Note in iunion() it is called that way because to avoid the implicit
! definitions, implicit real (...), in my_lfmm3dparttargmain()
!-------------------------------------------------------------------------

c234567 =============================================================

      ! This subroutine adds two vectors together (3 x N) in parallel
      ! c = a + b

      subroutine add_vectors(a, b, c, N, nthread)
...
c234567 =============================================================
      ...
      subroutine add_scalars(a, b, c, N, nthread)
...
c234567 =============================================================
      ! This subroutine takes in a complex vector (3 x N) and set it 
      ! to 0 in parallel

      subroutine zero_vector(vector, N, nthread)
...
c234567 =============================================================
      ! This subroutine takes in a complex scalar and sets it to 0 in 
      ! parallel

      subroutine zero_scalar(scalar, N, nthread)
...
c	Namdi: added the following:
	...
	!complex *16 myfldtarg_temp(3, ntarget)
	!complex *16 mypottarg_temp(ntarget)
...
c	Namdi: I added this
	...
	!write(*,*) "ifpot_m", ifpot_m
	!write(*,*) "ifpottarg_m", ifpottarg_m
	!write(*,*) "iffld_m", iffld_m
	!write(*,*) "iffldtarg_m", iffldtarg_m
c	
c     
...
c	----------------------------
	...
	!------------------------------------------
	! zero out source potential and field
	!------------------------------------------
...
c       
	...
	! zero out multirate potential and fields

	if (ifpot_m .eq. 1) then
c	  if (ifnear_only .ne. 1) then
	    call zero_scalar(potsrc_far, nsource, nthread)
	    call zero_scalar(potsrc_mpole, nsource, nthread)
c	  endif
	    call zero_scalar(potsrc_near, nsource, nthread)
	endif

	if (iffld_m .eq. 1) then
c	  if (ifnear_only .ne. 1) then
	    call zero_vector(fldsrc_far, nsource, nthread)
	    call zero_vector(fldsrc_mpole, nsource, nthread)
c	  endif
	    call zero_vector(fldsrc_near, nsource, nthread)
	endif

	if (ifpottarg_m .eq. 1) then
c	  if (ifnear_only .ne. 1) then
	    call zero_scalar(pottarg_far, ntarget, nthread)
	    call zero_scalar(pottarg_mpole, ntarget, nthread)
c	  endif
	    call zero_scalar(pottarg_near, ntarget, nthread)
	endif

	if (iffldtarg_m .eq. 1) then
c	  if (ifnear_only .ne. 1) then
	    call zero_vector(fldtarg_far, ntarget, nthread)
	    call zero_vector(fldtarg_mpole, ntarget, nthread)
c	  endif
	    call zero_vector(fldtarg_near, ntarget, nthread)
	endif

	! zero out stuff just for far field
c	if (ifnear_only .eq. 1 .and. ifpotsrc_near .eq. 1) then 
c	  call zero_scalar(pot, nsource, nthread)
c	endif

c	if (ifnear_only .eq. 1 .and. iffldsrc_near .eq. 1) then
c	  call zero_vector(fld, nsource, nthread)
c	endif

c	if (ifnear_only .eq. 1 .and. ifpottarg_near .eq. 1) then	    
c	    call zero_scalar(pottarg, ntarget, nthread)
c	endif

c	if(ifnear_only .eq. 1 .and. iffldtarg_near .eq. 1) then
c	  call zero_vector(fldtarg, ntarget, nthread)
c	endif
c
        do i=1,10
        timeinfo(i)=0
...
! definitions, implicit real (...), in my_lfmm3dparttargmain()
!-------------------------------------------------------------------------
	   subroutine my_lfmm3d_near_old( ier,iprec,
     $     ifevalfar,ifevalloc,
     $     nsource,sourcesort,isource,
     $     ifcharge,chargesort,
     $     ifdipole,dipstrsort,dipvecsort,
     ...
     $	   ifpottarg_near,
     $	   iffldtarg_near,
     $	   potsrc_far, fldsrc_far, 
     $ 	   potsrc_near, fldsrc_near,
     $	   pottarg_far, fldtarg_far,
     $	   pottarg_near,fldtarg_near)
...
c	Namdi: added the following:
	integer nthread
	integer ifpotsrc_near
	integer iffldsrc_near
	integer ifpottarg_near
	integer iffldtarg_near
	

	! multirate FMM flags for the potential and fields
	
	complex *16 potsrc_far(1), fldsrc_far(3,nsource) 
	complex *16 potsrc_near(1), fldsrc_near(3,nsource)

	complex *16 pottarg_far(1), fldtarg_far(3,ntarget) 
	complex *16 pottarg_near(1), fldtarg_near(3,ntarget)
...
c
	...
	! zero out multirate potential and fields
c	write(*,*) "ifpotsrc_near: ", ifpotsrc_near
c	write(*,*) "iffldsrc_near: " , iffldsrc_near
c	write(*,*) "nsource: ", nsource
...
ccC$OMP$PRIVATE(ibox,box,center0,corners0,nkids,list,nlist,npts)
ccC$OMP$PRIVATE(jbox,box1,center1,corners1)
ccC$OMP$PRIVATE(ier,ilist,itype) 
ccC$OMP$SCHEDULE(DYNAMIC)
cccC$OMP$NUM_THREADS(1) 
        do 7202 ibox=1,nboxes
c
        call d3tgetb(ier,ibox,box,center0,corners0,wlists)
        call d3tnkids(box,nkids)
c
...
c           call prinf('box=*',box,20)
c           call prinf('nkids=*',nkids,1)
c        endif
c
        if (nkids .eq. 0) then
            npts=box(15)
...
c
        if (nkids .eq. 0) then
c
c       ... evaluate self interactions (interactions within the same box)
c
        if( box(15) .gt. 0 ) then
...
c       ... evaluate interactions with the nearest neighbours
c
        ! Namdi: itype refers to list 1
        itype=1
        call d3tgetl(ier,ibox,itype,list,nlist,wlists)
c        if (ifprint .ge. 2) call prinf('list1=*',list,nlist)
c
c       ... for all pairs in list #1, 
c       evaluate the potentials and fields directly
c
            do 7203 ilist=1,nlist
               jbox=list(ilist)
               call d3tgetb(ier,jbox,box1,center1,corners1,wlists)
c
c       ... prune all sourceless boxes
c
         if( box1(15) .eq. 0 ) goto 7203
c
c---------------------------------------------------------------
c	Namdi: This is for evaluating the near-field interations that 
c	are solved directly
c	This function changes the field and potential of the 
c	sources.
c-------------------------------------------------------
...
ccC$OMP END PARALLEL DO
 ...
 9700   continue
c234567
	! This is to add the far and near potential/field
c	if(ifpotsrc_near .eq. 1) then
c	  call add_scalars(potsrc_far, potsrc_near, pot, nsource,
c     $ 		nthread)
c	endif
c
c	if(ifpottarg_near .eq. 1) then
c	  call add_scalars(pottarg_far, pottarg_near, pottarg, ntarget,
c     $		nthread)
c	endif

c	if(iffldsrc_near .eq. 1) then
c	  call add_vectors(fldsrc_far, fldsrc_near, fld, nsource, 
c     $ 		nthread)
c	endif
	
c	if(iffldtarg_near .eq. 1) then
c	  call add_vectors(fldtarg_far, fldtarg_near, fldtarg, ntarget, 
c     $ 		nthread)
c	endif

	print *, "leaving my_lfmm3d_near()"
        return
        end