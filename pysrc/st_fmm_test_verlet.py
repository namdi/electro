...
def run(iprec, ifpot, iffld, ifcharge, ifdipole, ifpottarg, iffldtarg, 
  ...
  for i in range(nsteps+1): # loop through nsteps + 1 so I can get the velocity for i==nsteps
    ...
    if (iffmm == 1 or iffm_multi == 1):
      ...
      #update the acceleartion FMM
      mu.updateAccel(bsource, btarget, accel_src, accel_old_src, field_src, accel_targ, accel_old_targ, field_targ, charge_src_reshape)
      
      # update velocity FMM using velocity Verlet integration
      
      if i > 0:
	mu.updateVel(bsource, btarget, vel_src, accel_src, accel_old_src, vel_targ, accel_targ, 
		  accel_old_targ, dt)
       
      # Update positions using velocity Verlet integration
      mu.updatePos(bsource, btarget, pos_src, vel_src, accel_src, pos_targ, vel_targ, accel_targ, dt)
    ...
    if (ifdirect == 1):
      ...
      mu.updateAccel(bsource, btarget, d_accel_src, d_accel_old_src, d_field_src, d_accel_targ, d_accel_old_targ, 
		  d_field_targ, charge_src_reshape)
      # testing
      #print "\ni: " + str(i)
      #print d_accel_old		  
      ...
      if i > 0:
	mu.updateVel(bsource, btarget, d_vel_src, d_accel_src, d_accel_old_src, d_vel_targ, d_accel_targ, 
		  d_accel_old_targ, dt)
		  
      # update positions using velocity Verlet integration
      mu.updatePos(bsource, btarget, d_pos_src, d_vel_src, d_accel_src, d_pos_targ, d_vel_targ, d_accel_targ, dt)
    ...
    if save is True:
      ...
      # save the position. Do not save the position for interation i == nsteps
      if i < nsteps:
	mu.save_pos(newpath, pos_src, pos_targ, d_pos_src, d_pos_targ)
